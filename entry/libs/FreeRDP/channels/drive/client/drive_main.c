...
/*
 * [OHOS adaptation] File-open observation hook.
 *
 * Lets a wrapper library (libfreerdp_harmonyos) observe plain read-only
 * FILE_OPEN requests the server issues against the redirected drive
 * (e.g. Explorer double-click on a document). The wrapper then opens the
 * file locally on the client device. Purely observational: the IRP is
 * always completed normally, so remote behaviour is unchanged.
 *
 * Enabled via CMake option WITH_OHOS_FILE_OPEN (default OFF); compiled out
 * entirely on other platforms / when the option is off. The setter is
 * exported with default visibility so it can be resolved with dlsym from
 * the wrapper (channel symbols are hidden in this build).
 */
typedef void (*ohos_freerdp_file_open_cb)(int64_t instance, const char* fullpath,
                                          const char* filename);

static ohos_freerdp_file_open_cb g_ohosFileOpenCallback = NULL;

FREERDP_API void ohos_freerdp_set_file_open_callback(ohos_freerdp_file_open_cb cb)
{
	g_ohosFileOpenCallback = cb;
}

/* Notify debounce interval in ms: Explorer often opens the same file more
 * than once in quick succession (icon extraction, preview handlers). */
#define OHOS_FILE_OPEN_DEBOUNCE_MS 3000

static BOOL ohos_is_meta_file_name(const char* filename)
{
	if (!filename)
		return FALSE;
	if (_stricmp(filename, "desktop.ini") == 0)
		return TRUE;
	if (_stricmp(filename, "thumbs.db") == 0)
		return TRUE;
...
static void ohos_notify_file_open(DRIVE_DEVICE* drive, const DRIVE_FILE* file)
{
	WINPR_ASSERT(drive);
	WINPR_ASSERT(file);

	if (!g_ohosFileOpenCallback || !drive->rdpcontext)
		return;

	/* Server-side directory opens are irrelevant for local file opening. */
	if (file->is_dir)
		return;

	const char* filename = NULL;
	char* fullpath = ConvertWCharToUtf8Alloc(file->fullpath, NULL);
	if (!fullpath)
		return;

	/* Debounce: repeated opens of the same path within the interval are
	 * dropped (icon/preview noise). */
	const UINT64 now = GetTickCount64();
	if (strcmp(fullpath, drive->ohosLastFileOpenPath) == 0 &&
	    (now - drive->ohosLastFileOpenTick) < OHOS_FILE_OPEN_DEBOUNCE_MS)
	{
		free(fullpath);
		return;
	}
	...
	if (ohos_is_meta_file_name(filename))
	{
		free(fullpath);
		return;
	}

	strncpy(drive->ohosLastFileOpenPath, fullpath, sizeof(drive->ohosLastFileOpenPath) - 1);
	drive->ohosLastFileOpenPath[sizeof(drive->ohosLastFileOpenPath) - 1] = '\0';
	drive->ohosLastFileOpenTick = now;

	g_ohosFileOpenCallback((int64_t)(uintptr_t)drive->rdpcontext->instance, fullpath, filename);
	free(fullpath);
}
#endif
